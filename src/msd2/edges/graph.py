...
"""
Referencing MSD Github [here](https://github.com/caspervanengelenburg/msd/blob/main/NB%20-%20Data%20Curation%202%20-%20Graph%20Extraction.ipynb)
"""
...
def polygon_to_array(polygon: Polygon):
    """Converts a polygon into a numpy array."""
    return np.array(polygon_to_list(polygon))


def extract_access_graph(
    geoms: list[Polygon],
    geoms_type: list[str],
    id: int,
    classes: list[str] = ROOM_NAMES,
):
    ...
    for key, (area, area_type) in enumerate(zip(areas, areas_type)):
        ...
        if "Zone1" in classes:
            area_nodes[key] = {"zoning_type": mapping[area_type]}
    ...
    for (i, v1), (j, v2) in combinations(enumerate(areas), 2):
        ...
        # Option 2: DOOR
            ...
            for door in doors:
                if door.distance(v1) < 0.05 and door.distance(v2) < 0.05:
                    ...
                    edges.append(
                        [i, j, {"connectivity": "door"}]
                    )  # , 'door_geometry': polygon_to_list(door)}])
                else:
                    continue
        ...
        for entrance_door in entrance_doors:
            if entrance_door.distance(v1) < 0.05 and entrance_door.distance(v2) < 0.05:
                # Adds the geometry of the door as well (slightly different from paper)
                edges.append(
                    [i, j, {"connectivity": "entrance"}]
                )  # , 'door_geometry': polygon_to_list(entrance_door)}])
            else:
                continue
...
def get_geometries_from_id(df, floor_id, column="zoning"):
    ...
    df_floor = df[(df["floor_id"] == floor_id)]
